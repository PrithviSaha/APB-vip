...
class apb_scoreboard extends uvm_scoreboard;
	...
	function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual apb_if)::get(this, "", "vif", vif)) begin
			`uvm_fatal("SCOREBOARD", "Virtual interface must be set for scoreboard")
		end
	endfunction
	...
	task compare(apb_sequence_item inp_item, apb_sequence_item out_item);
		if(!vif.PRESETn) begin
			`uvm_info("SCOREBOARD", $sformatf("RESET : reset = %b , Data=0x%0h, PSLVERR = %b", vif.PRESETn, out_item.apb_read_data_out, 
				         out_item.PSLVERR), UVM_MEDIUM)

			if(out_item.apb_read_data_out == 0 && out_item.PSLVERR == 0 )begin
